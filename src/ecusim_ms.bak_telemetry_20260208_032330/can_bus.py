...
from ecusim_ms.transport import CanTransport, PythonCanTransport, SlcanSerialTransport, TermuxUsbSlcanTransport
...
class CanBus:
    ...
    def open(self) -> None:
        ...
        if self.backend == "pythoncan":
            if self.iface == "termux-usb":
                self.transport = TermuxUsbSlcanTransport(bitrate=self.bitrate)
            else:
